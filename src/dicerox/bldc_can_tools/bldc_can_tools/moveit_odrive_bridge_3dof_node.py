...
JOINT_MAPPINGS: tuple[JointMapping, ...] = (
    JointMapping("Joint1", "1", "0x10", -90.0, 90.0),
    JointMapping("Joint2", "2", "0x11", 0.0, 180.0),
    JointMapping("Joint3", "3", "0x12", -200.0, 0.0),
)


class MoveItOdriveBridge3DofNode(Node):  # type: ignore[misc]
    def __init__(self) -> None:
        ...
        super().__init__("moveit_odrive_bridge_3dof")
        ...
        summary = ", ".join(
            f"{mapping.moveit_name}->{mapping.node_label}[{mapping.min_deg:.0f},{mapping.max_deg:.0f}]"
    ...
    def _publish_latest_targets(self) -> None:
        ...
        if age_sec > self.joint_state_timeout_sec:
            if not self._timeout_warned:
                action = "sending IDLE to joints 1-3" if self.idle_on_stale else "holding last targets"
        ...
        desired_triplet: list[float] = []
        commanded_triplet: list[float] = []
        any_change = False
        for mapping in JOINT_MAPPINGS:
            ...
            desired_triplet.append(target_deg)
            ...
            commanded_triplet.append(current_deg)
        ...
        if len(desired_triplet) != len(JOINT_MAPPINGS):
            return

        if not any_change:
            return

        if not self._send_joint_triplet(commanded_triplet):
            return

        for mapping, target_deg in zip(JOINT_MAPPINGS, commanded_triplet):
            self._commanded_deg_by_joint[mapping.moveit_name] = target_deg
            self._last_sent_deg_by_joint[mapping.moveit_name] = target_deg

    def _send_joint_triplet(self, command_deg_triplet: list[float]) -> bool:
        if self._serial is None:
            raise RuntimeError("Serial port is not open")

        try:
            command_line = "j " + " ".join(f"{target_deg:.3f}" for target_deg in command_deg_triplet) + "\n"
            self._serial.write(command_line.encode("ascii"))
            self._serial.flush()
        except SerialException as exc:
            self.get_logger().error(f"Failed to write MoveIt triplet: {exc}")
            return False

        summary = ", ".join(
            f"{mapping.moveit_name}={target_deg:.2f}"
            for mapping, target_deg in zip(JOINT_MAPPINGS, command_deg_triplet)
        )
        self.get_logger().info(f"MoveIt triplet sent: {summary}", throttle_duration_sec=0.5)
        return True
    ...
    def _send_idle_to_all_joints(self) -> None:
        ...
        try:
            ...
            self.get_logger().error(f"Failed to send IDLE to joints 1-3: {exc}")
            return
...
def main(argv: list[str] | None = None) -> None:
    ...
    node = MoveItOdriveBridge3DofNode()
    try:
        rclpy.spin(node)
    finally:
        node.destroy_node()
